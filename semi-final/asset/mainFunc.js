        ...
        if(data.total_pages > 1 && pageNum < maxPageNum){
          ...
          .then(()=>{
            ...
            fetch(trendyPersonApiUrl)
              .then(res => res.json())
              .then(personTrendy=>{
                for (let i = 0; i < personTrendy.results.length/2; i++) {
                  trendyPersonList += `
                  <div class='trendy-person-container'>
                    <p class="trendy-person-name" >${personTrendy.results[i].name}</p>
                    <img src = 'https://image.tmdb.org/t/p/original${personTrendy.results[i].profile_path}' alt="movie star's profile picture" id="${personTrendy.results[i].id}" class='main-trendy-poster'>
                  </div>
                  `;
                }
  ...
  fetch(apiUrl)
    ...
    .then(data => {
      ...
      .then(()=>{
        ...
        // Initiate scatterplot
        let trendyScatterPlot = new ScatterPlot('movie-scatterplot-container', dataFiltered, 480, 380);
        trendyScatterPlot.render();
